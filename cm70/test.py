...
def detect_motion(frame, bg_subtractor, kernel, min_contour_area=8000):
    # Apply background subtraction
    fg_mask = bg_subtractor.apply(frame, learningRate=0.005)  # Adjust learning rate as needed
    fg_mask = cv2.morphologyEx(fg_mask, cv2.MORPH_OPEN, kernel)
    fg_mask = cv2.morphologyEx(fg_mask, cv2.MORPH_CLOSE, kernel)

    # Apply thresholding to clean up the mask
    _, fg_mask_thresh = cv2.threshold(fg_mask, 50, 255, cv2.THRESH_BINARY)
...
def record_video(cap, lock, filename, duration=20, bg_subtractor=None, kernel=None):
    ...
    while time.time() - start_time < duration:
        ...
        if ret:
            ...
            motion_detected, processed_frame, object_size, position = detect_motion(frame, bg_subtractor, kernel)
...
def stabilize_camera(cap, bg_subtractor, kernel, stabilization_time=5):
    ...
    while time.time() - start_time < stabilization_time:
        ...
        if not ret:
            ...
            break
        # Feed frames to background subtractor without motion detection
...
def main():
    ...
    # Stabilize the camera and background subtractor
    stabilize_camera(cap, bg_subtractor, kernel, stabilization_time=5)
    ...
    while True:
        ...
        if motion_detected and not recording:
            ...
            threading.Thread(target=record_video, args=(cap, lock, filename, 20, bg_subtractor, kernel)).start()
